...
<script lang="ts">
    ...
    const appName = Config.get('applicationName') || 'AtroCore';
    const description = Language.translate('passwordChangeRequestForm', 'messages', 'User').replace('{appName}', appName);
    ...
    <LogoCard>
        <div class="body">
            {#if success}
                ...
                <a href={loginUrl} role="button" class="primary submit-button">
                    <i class="ph ph-sign-in"></i>
                    <span>{Language.translate('Login', 'labels', 'User')}</span>
                </a>
            {:else if requestInvalid}
                <div class="error-message">
                    {Language.translate('passwordChangeRequestNotFound', 'messages', 'User')}
                </div>
            {:else}
